...
describe("GET /rickandmorty/login", () => {
    it('La ruta de login retorna access true', async () => {
    const response = (await agent.get('/rickandmorty/login?email=angeles@gmail.com&password=123456')).body;
    expect(response.access).toEqual(true);
    })
    it('La ruta de login retorna access false', async () => {
        const response = (await agent.get('/rickandmorty/login?email=angelescom&password=123456')).body;
...
describe("POST /rickandmorty/fav", () => {
    ...
    it('Devuelve un arreglo enviado por body', async () => {
        const response = (await agent.post('/rickandmorty/fav').send(character1)).body;
        expect(response).toContainEqual(character1) 
    })
    it('Devuelve un arreglo enviado por body con los 2 personajes', async () => {
...
describe("DELETE /rickandmorty/fav/:id", () => {
    ...
    it('Devuelve los personajes existentes luego de ID invalido', async () => {
        const response = (await agent.delete('/rickandmorty/fav/9999')).body;
        expect(response).toContainEqual(character1) 
        expect(response).toContainEqual(character2) 
    })
